  ...
  try {
    await withProject(
      async ({ slug }) => {
        ...
        await expect(page).toHaveURL(new RegExp(`/v1/${otherSlug}/session`))
        ...
        await expect
          .poll(() => {
            ...
            if (new RegExp(`^/v1/${slug}/session(?:/[^/]+)?/?$`).test(pathname)) return "project"
            if (pathname === "/v1") return "home"
            return ""
          })
          .toMatch(/^(project|home)$/)

        await expect(page).not.toHaveURL(new RegExp(`/v1/${otherSlug}/session(?:[/?#]|$)`))
